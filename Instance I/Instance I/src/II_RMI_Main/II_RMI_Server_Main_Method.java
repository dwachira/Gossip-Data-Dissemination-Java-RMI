...
public class II_RMI_Server_Main_Method {
    ...
    int Counter_for_Spreading_Infection = 0;
    int Instance_Selected_Randomly = 0;
    
    Integer[] a = { 2, 3};
    int[] Array_Instance_Selected_Randomly = new int[a.length];
    String Msg_to_be_Passed = "";
    
    private void Start_RMI_Server(){
        try { 
            Log_Information_to_Text_file ("Starting Server - Instance 1");
      /********************************************************
      *                                                       *
      *   The Port assigned is Port Number 2201               *
      *                                                       *
      ********************************************************/
            Registry registry = LocateRegistry.createRegistry(2201);
            Log_Information_to_Text_file ("Port 2201 dedicated for the Creation of a New Service"); 
      ...
      *                                                       *
      ********************************************************/
            registry.rebind("New_Message_Service_Instance_1", new II_Message_Implementation_at_Server());
            Log_Information_to_Text_file ("New_Message_Service created");
            
        } catch (Exception e) {
            e.printStackTrace();
            Log_Information_to_Text_file ("Error within Start_RMI_Server() --> "+e);
        } 
        Log_Information_to_Text_file ("Instance I is Ready; Listening on Port 2201");
        Msg_to_be_Passed = JOptionPane.showInputDialog(null, "What's the message you want to pass around?", "Gossip Dissemination - Instance I", JOptionPane.QUESTION_MESSAGE);
   ...
   public void Test_Message_Implementation()
   {
      ...
      ********************************************************/        
            ...
            String Message_to_be_Communicated = Msg_to_be_Passed + " [Original - Instance I]";
    ...
    private void Check_Infection_Spread()
    {
          int i=JOptionPane.showConfirmDialog(null, "Do you want to initiate an epidemic?","Really",JOptionPane.YES_NO_OPTION);
            // TODO add your handling code here:
          if(i==JOptionPane.YES_OPTION)
          {
                    while (Counter_for_Spreading_Infection<= 2)//array_size_number_of_connected_nodes)
                    {
                        //Counter_for_Spreading_Infection++;
                        Test_Message_Implementation();
                    }     
          }
          else
          {
